...
class Controller {
    ...
    basicFetch(path, body) {
        return isomorphic_fetch_1.default(path, this.basicFetchOptions(body, any))
            .then(res => res.text());
    }
    // Rest fetch
    controllerFetch(method, path, body) {
        return isomorphic_fetch_1.default(`${this.server()}${path}`, this.fetchOptions(method, string, body, any))
            .then(res => res.json())
